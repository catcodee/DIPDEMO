...
void ImageMat::FullFill(vector<CONNECTDOMAIN> &C)
{
	int i = 0;
	int j = 0;
	IMAGEDATA color;

	for (auto iters1 = C.begin(); iters1 < C.end(); iters1++)
	{
		color.blue = unsigned char(rand());
		color.green = unsigned char(rand());
		color.red = unsigned char(rand());
		for (auto iters2 = (*iters1).D.begin(); iters2 < (*iters1).D.end(); iters2++)
		{
			ColorMat[(*iters2).x][(*iters2).y] = color;
...
void ImageMat::FindConnectedDomain(vector<CONNECTDOMAIN> &C)
{
	...
	for (i = 1; i < Height - 1; i++) //检查每个像素的左侧，正下和右侧
	{
		for (j = 1; j < Width - 1; j++)
		{
			if (Mat[i][j] == 255 && LabelMat[i][j] == 0) //未被标记的白色初始点
			{
				...
				tmpConnectDomain.Label = CurrentLabelNum;
				C.push_back(tmpConnectDomain);
				tmpConnectDomain.D.clear();
				CurrentLabelNum++;
			}
		}
	}
